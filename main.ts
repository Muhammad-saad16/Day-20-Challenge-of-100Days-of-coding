...
function addNumber(valueToAdd: number): (number :number) => number {
...
function profile(user: {name: string, age: number}) {
    console.log(`My name is ${user.name} and I am ${user.age} years old`);
}

profile(user);  //output is My name is Muhammad Saad and I am 16 years old
